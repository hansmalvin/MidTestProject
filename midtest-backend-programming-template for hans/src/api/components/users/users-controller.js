...
async function getUsers(request, response, next) {
  try {
    ...
    const users = await usersService.getUsers(page_number,
       page_size, search, sort);
...
/**
 * Limit login attempts and handle login process
 * @param {object} request - Express request object
 * @param {object} response - Express response object
 * @param {object} next - Express route middlewares
 * @returns {object} Response object or pass an error to the next route
 */
async function loginLimit(request, response, next) {
  const email = request.body.email;
  const password = request.body.password;
  const password_confirm = request.body.password_confirm
  ...
  try {
    ...
    await usersService.loginCheck(email);
    const lifePoints = await usersService.getAttempts(email);
    if(lifePoints >= 5){
        throw errorResponder(
    ...
    if(loginSuccess){
      await usersService.resetAttempts(email);
    }
